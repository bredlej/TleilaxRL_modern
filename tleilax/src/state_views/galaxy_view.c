  ...
  for (uint32_t sectorZ = 0; sectorZ < AMOUNT_SECTORS_Z; sectorZ++) {
    for (uint32_t sectorY = 0; sectorY < AMOUNT_SECTORS_Y; sectorY++) {
      for (uint32_t sectorX = 0; sectorX < AMOUNT_SECTORS_X; sectorX++) {
        struct Star *starInSectorXYZ = StarAt(sectorX + floorf(Galaxy.offset.x),
        ...
        if (starInSectorXYZ) {
          ...
          Vector3 starOutlineSize = {starInSectorXYZ->size, starInSectorXYZ->size, starInSectorXYZ->size};
          ...
          if (isMousePointerOnStar) {
            ...
            DrawSphere(starPositionInWorldCoords, starInSectorXYZ->size, RAYWHITE);
            DrawCubeWires(starPositionInWorldCoords, 1.0f, 1.0f, 1.0f, GREEN);

            EndMode3D();
            char textAboveStar[20];
            PrepareStarDescription(xOffset, yOffset, zOffset, sectorZ, sectorY, sectorX, starInSectorXYZ,
                                   textAboveStar);
            DescribeStar(camera, &starPositionInWorldCoords,
                         textAboveStar);
            BeginMode3D((*camera));
...
void DescribeStar(Camera *camera, Vector3 *starPositionInWorldCoords,
                  const char *starDescription) {
  Vector2 starPositionInScreenCoords =
